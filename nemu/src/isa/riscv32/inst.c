...
#define immU() do { *imm = SEXT((BITS(i, 31, 12)  << 12), 20); printf("imm = %d\n",*imm);} while(0)
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = src1 >> (src2 & 0x1f));
  ...
  INSTPAT("??????? ????? ????? 011 ????? 01100 11", sltu   , R, R(rd) = (word_t)src1 < (word_t)src2 ? 1 : 0);
  INSTPAT("??????? ????? ????? 010 ????? 00100 11", slti   , I, R(rd) = src1 < imm ? 1 : 0);
  INSTPAT("??????? ????? ????? 000 ????? 01100 11", sub    , R, R(rd) = src1 - src2);
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, if (src2 == 0) R(rd) = -1; else R(rd) = (word_t)src1 / (word_t)src2);
